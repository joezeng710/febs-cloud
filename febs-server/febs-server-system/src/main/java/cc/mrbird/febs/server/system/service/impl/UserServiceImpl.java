...
public class UserServiceImpl extends ServiceImpl<UserMapper, SystemUser> implements IUserService {
    ...
    @Override
    @Transactional
    public FebsResponse createUser(SystemUser user) {
        IPage<SystemUser> userDetail = findUserDetail(user, new QueryRequest());
        if ( userDetail.getRecords() != null) {
            return new FebsResponse().message("用户名已存在");
        }
        ...
        return new FebsResponse().message("新增成功");
    }
